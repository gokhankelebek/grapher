...
import type { InequalityOutcome, NLItem } from '../types'
...
function fail(message: string, pos?: number): never {
  throw new IneqError(message, pos)
}
...
function checkBalanced(src: string): void {
  ...
  for (let i = 0; i < src.length; i++) {
    const c = src[i]
    if (OPENERS.includes(c)) stack.push({ c, at: i })
    else if (CLOSERS.includes(c)) {
      if (stack.length === 0) fail(`Unmatched '${c}' at position ${i}`, i)
      stack.pop()
    }
  }
  if (stack.length > 0) {
...
/** Shared state for one parse: which variable this number line is about. */
interface Ctx {
...
function noteVariable(ctx: Ctx, name: string, tex: string, pos: number): void {
 ...
 */
function classifyOperand(raw: Span, ctx: Ctx, missing: string): Operand {
  ...
  if (SINGLE_LETTER_RE.test(s.text)) {
...
function boundOperand(raw: Span, ctx: Ctx, missing: string): Bound {
...
function scanRels(s: string): RelTok[] {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c === '!') {
      ...
      fail(`Unexpected '!' at position ${i} — did you mean '!=' ?`, i)
    }
    if (c === '<') {
      if (s[i + 1] === '=') { out.push({ op: '<=', at: i, len: 2 }); i++; continue }
      if (s[i + 1] === '>') { out.push({ op: '!=', at: i, len: 2 }); i++; continue }
...
function parseClause(raw: Span, ctx: Ctx): Piece[] {
  const s = trimSpan(raw)
...
function parseSet(inner: Span, closer: string, whole: Span, ctx: Ctx): Piece[] {
...
function parseChain(s: Span, ctx: Ctx): Piece[] {
  const rels = scanRels(s.text)
  if (rels.length === 0) {
    fail(
...
function toItems(pieces: Piece[]): NLItemDraft[] {
...
/** Interval / set notation for the card, with the bounds as they were typed. */
function toLatex(pieces: Piece[], ctx: Ctx): string {
  ...
  return ctx.tex ? `${ctx.tex} \\in ${body}` : body
}
...
export function parseInequality(src: string): InequalityOutcome {
  try {
    ...
    const pieces = union(groups.flat())
    const items: NLItemDraft[] = toItems(pieces)
    return { ok: true, items, latex: toLatex(pieces, ctx) }
  } catch (err) {
    if (err instanceof IneqError) {
